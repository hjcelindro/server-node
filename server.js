    <head>
        ...
        <script type="text/javascript">
            ...
            socket.on('mqtt',function(msg){
                ...
                $('table').append('<tr id="'+id+'" class="rowID" ><td>'+id+'</td><br><td>'+String(msg.payload.location)+'</td><br><td>'+String(msg.payload.manufacturer)+'</td><br><td>'+String(msg.payload.message)+'</td><br><td align="center"><input value="collect" type="button" class="collect" onclick=getResponse($(this).closest("tr").attr("id")+"/"+this.value) /><input value="ignore" type="button" class="ignore" onclick=getResponse($(this).closest("tr").attr("id")+"/"+this.value)/></td></tr>');
...
/*            //function getItem(clickedID){
                $('input[type=button]').click(function(){
                    rowID = $(this).closest('tr').attr('id');
                    window.alert("ID:"+ rowID);
                    socket.emit('client response',rowID);
                });
            //}
            */
            function getResponse(data){
                //window.alert(data);
                confirm("Collect item with RFID: "+data);
